 ...
 *
 * On the published static site this is a read-only reference table: there is no
 * server to accept a form post, so the inputs are replaced by plain values.
 */
export default function CriteriaPage() {
    ...
    return (
      <tr key={c.id}>
        <td>
          ...
          <div className="muted mt-1 text-xs"><code>{c.key}</code></div>
        </td>
        <td>
          {IS_STATIC
        ...
        <td>
          {IS_STATIC
            ? <span className="muted">{c.required ? 'Yes' : '—'}</span>
      ...
      <table className="grid">
        <thead>
          <tr>
            ...
            {!IS_STATIC && <th style={{ width: 90 }}></th>}
          </tr>
        </thead>
        <tbody>{rows}</tbody>
      </table>
  ...
  return (
    <>
      ...
      {IS_STATIC ? (
        <div className="card mb-4 overflow-x-auto">{table}</div>
      ) : (
        <form action={saveCriteria} className="card mb-4 overflow-x-auto">{table}</form>
      )}
