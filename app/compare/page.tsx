...
import { buildRankings, resolveTrip, type RankingRow } from '@/lib/view';
...
/** Index of the row minimizing `pick`, ignoring nulls. */
...
export default async function ComparePage({
  searchParams,
}: { searchParams: Promise<{ ids?: string; trip?: string }> }) {
  const params = await searchParams;
  const trip = resolveTrip(params.trip);
  const { rows: allRows, criteria } = buildRankings(trip);
    ...
    return (
      <div className="card p-8 text-center">
        ...
        <p className="muted mb-4 text-sm">
          Tick two or more resorts on the Rankings page, then choose “Compare side by side”.
        </p>
        <Link className="btn btn-primary" href={trip ? `/?trip=${trip.id}` : '/'}>
          Go to rankings ({db.resorts.length} resorts)
        </Link>
      </div>
    );
  }
